...
`xdamnprd` user. To limit filesystem access for jobs they are always run inside
a [sandbox](https://git.xfel.eu/dataAnalysis/proposal-sandbox) that executes the
context file under the `xdamntst` user with dynamic privileges to only allow
access to their proposal.
...
You could get the status of the listener with:
```bash
...
launch jobs for all proposals you can change a setting in its database:
```bash
